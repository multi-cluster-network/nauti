...
func DeprecatedPatchPodAnnotationsWithRetry(client kubernetes.Interface, pod *v1.Pod, annoKeys, annoValues []string) error {
	return retry.RetryOnConflict(retry.DefaultBackoff, func() (err error) {
		cachedPod := pod.DeepCopy()
		if pod.GetAnnotations() == nil {
			pod.Annotations = make(map[string]string)
		}
		for i, annoKey := range annoKeys {
			pod.Annotations[annoKey] = annoValues[i]
		}
...
func SetSpecificAnnotations(client kubernetes.Interface, pod *v1.Pod, annotationKeys, annotationValues []string, override bool) error {
	return retry.RetryOnConflict(retry.DefaultBackoff, func() (err error) {
		cachedPod := pod.DeepCopy()
